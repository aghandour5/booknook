...
if ($action === "delete_book") {
  if ($_SERVER["REQUEST_METHOD"] !== "POST") redirect("admin.php?tab=books");
  $id = (int)($_POST["id"] ?? 0);
  if ($id > 0) $pdo->prepare("DELETE FROM books WHERE id=?")->execute([$id]);
  redirect("admin.php?tab=books");
}
...
if ($action === "add_book") {
  ...
  $u->execute([$userId]);
  if (!$u->fetch()) redirect("admin.php?tab=books");
  ...
  if ($isbnNorm === "" || $title === "" || $author === "") redirect("admin.php?tab=books");
...
<div class="topbar">
  ...
  <div class="btn-row">
    <a class="btn <?= $tab==="users" ? "btn-primary" : "" ?>" href="admin.php?tab=users">Users</a>
    <a class="btn <?= $tab==="books" ? "btn-primary" : "" ?>" href="admin.php?tab=books">Books</a>
  </div>
</div>
...
<div class="panel">
      ...
      <tbody>
        <?php foreach ($users as $x): ?>
          <tr>
            ...
            <td class="text-right">
              <form method="post" action="admin.php?tab=users&action=delete_user" class="delete-form" onsubmit="return confirm('Delete this user?')">
  ...
  <?php else: ?>
    <div class="notice add-book-notice">
      ...
      <form method="post" action="admin.php?tab=books&action=add_book" class="add-book-form">
        <input type="hidden" name="csrf" value="<?= h(csrf_token()) ?>">
      ...
      <tbody>
        <?php foreach ($books as $x): ?>
          <tr>
            ...
            <td><?= h($x["created_at"]) ?></td>
            <td class="text-right">
              <form method="post" action="admin.php?tab=books&action=delete_book" class="delete-form" onsubmit="return confirm('Delete this book?')">
                <input type="hidden" name="csrf" value="<?= h(csrf_token()) ?>">
                <input type="hidden" name="id" value="<?= (int)$x["id"] ?>">
    ...
    <?php foreach ($comments as $c): ?>
      <div class="notice comment">
        ...
        <form method="post" action="admin.php?tab=books&action=delete_comment" class="delete-comment-form" onsubmit="return confirm('Delete this comment?')">
          <input type="hidden" name="csrf" value="<?= h(csrf_token()) ?>">
